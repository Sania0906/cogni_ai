  ...
  try {
    ...
    return res.json({
      metrics: [
        {
          ...
          android: Math.round(totalUsers * 0.4),
          web: Math.round(totalUsers * 0.6),
          unit: "users",
        },
        {
          name: "Resumes Uploaded",
          android: Math.round(totalResumes * 0.35),
          web: Math.round(totalResumes * 0.65),
          unit: "documents",
        },
        {
          name: "Assessments Completed",
          android: Math.round(totalAssessments * 0.3),
          web: Math.round(totalAssessments * 0.7),
          unit: "tests",
        },
        {
          name: "Average Page Load Speed",
          android: 1.2,
          web: 0.85,
          unit: "seconds",
        },
      ...
      platformGrowth: [
        {
          month: "Jan",
          android: Math.round(totalUsers * 0.3),
          web: Math.round(totalUsers * 0.5),
        },
        {
          month: "Feb",
          android: Math.round(totalUsers * 0.35),
          web: Math.round(totalUsers * 0.55),
        },
        {
          month: "Mar",
          android: Math.round(totalUsers * 0.4),
        ...
        {
          month: "Apr",
          android: Math.round(totalUsers * 0.45),
          web: Math.round(totalUsers * 0.65),
        },
        {
          month: "May",
          android: Math.round(totalUsers * 0.5),
          web: Math.round(totalUsers * 0.7),
        },
      ],
      geographicStats: [
    ...
    return res
      .status(500)
      .json({ message: "Failed to load live platform analytics" });
  }
});
