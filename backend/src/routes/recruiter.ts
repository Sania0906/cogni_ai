...
router.get("/candidates", authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  const filterSkills = skills ? (skills as string).split(",").map(s => s.toLowerCase().trim()) : [];
  ...
  try {
    ...
    let candidates = profiles.map((p: any) => {
      ...
      return {
        ...p,
        ats_score: resume?.metadata?.ats_score || Math.floor(Math.random() * 40) + 40, // fallback for mock
    ...
    if (filterSkills.length > 0) {
      candidates = candidates.filter((c: any) => 
        filterSkills.every(fs => c.skills.some((cs: string) => cs.toLowerCase().includes(fs)))
      );
    }

    if (filterExp > 0) {
      candidates = candidates.filter((c: any) => c.experience_years >= filterExp);
    }
    ...
    const shortlistedSet = new Set(shortlists?.map((s: any) => s.candidate_id) || []);
...
// 2. TOGGLE SHORTLIST
// =========================================================================
router.post("/shortlist", authMiddleware, async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  if (!userId) return res.status(401).json({ message: "Unauthorized" });
...
// =========================================================================
router.get("/shortlist", authMiddleware, async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  if (!userId) return res.status(401).json({ message: "Unauthorized" });
  ...
  try {
    ...
    const candidates = profiles.map((p: any) => {
      ...
      return {
        ...p,
        ats_score: resume?.metadata?.ats_score || Math.floor(Math.random() * 40) + 40,
        skills: resume?.skills || [],
        experience_years: totalExpYears || Math.floor(Math.random() * 5) + 1,
        latest_resume_id: resume?.id,
        is_shortlisted: true
