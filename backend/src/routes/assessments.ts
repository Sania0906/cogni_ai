...
> = {
  programming: [
    ...
    {
      q: "Which of the following is a key feature of Python?",
      options: [
        "Static typing",
    ...
    {
      q: "What does the 'self' keyword represent in Python class methods?",
      options: [
  ...
  database: [
    ...
    {
      q: "What does 3NF stand for in database normalization?",
      options: [
        "Third Normal Form",
        "Triple Network File",
        "Three-Node Framework",
        "Third Nominal File",
      ],
      correct: 0,
    },
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const hasAI = combinedTokens.some(
        (t) =>
          t.includes("ai") ||
          t.includes("ml") ||
          t.includes("machine") ||
      ...
      const hasWeb = combinedTokens.some(
        (t) =>
          t.includes("web") ||
          t.includes("html") ||
          t.includes("css") ||
      ...
      const hasDB = combinedTokens.some(
        (t) =>
          t.includes("db") ||
          t.includes("sql") ||
          t.includes("postgres") ||
      ...
      const hasCloud = combinedTokens.some(
        (t) =>
          t.includes("cloud") ||
          t.includes("aws") ||
          t.includes("azure") ||
      ...
      const hasProg = combinedTokens.some(
        (t) =>
          t.includes("python") ||
          t.includes("java") ||
          t.includes("c++") ||
...
router.get("/", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: err.message || "Failed to retrieve assessments" });
  }
});
...
router.post("/", authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: err.message || "Failed to submit assessment" });
  }
});
