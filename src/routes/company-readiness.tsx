...
import { Building, Briefcase, CheckCircle, XCircle, ArrowRight, Loader2, Sparkles, Target, AlertTriangle, ShieldCheck } from "lucide-react";
...
const COMPANIES = [
  ...
  "TCS", "Infosys", "Accenture", "IBM", "Cognizant", "Zoho"
];

const ROLES = [
  "Software Engineer", "AI Engineer", "Data Scientist", "Full Stack Developer", 
...
function CompanyReadiness() {
  ...
  useEffect(() => {
    ...
    api.getLatestCompanyReadiness().then(res => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const result = await api.generateCompanyReadiness(selectedCompany, selectedRole);
      setReport(result);
    ...
    <AppShell>
      <header className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Compare your uploaded resume against specific company and role requirements.
        </p>
      </header>
          ...
          {analyzing ? (
            <div className="h-full min-h-[400px] flex flex-col items-center justify-center p-8 rounded-3xl bg-card border border-border/10 shadow-card">
              ...
              <p className="mt-6 text-lg font-bold animate-pulse text-center">
                Comparing your resume against {selectedCompany}'s hiring heuristics for {selectedRole}...
              </p>
            </div>
          ) : report ? (
            <div className="space-y-6">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
                  <div className="relative h-32 w-32 flex items-center justify-center shrink-0">
                    <svg className="h-full w-full transform -rotate-90">
                      <circle cx="64" cy="64" r="56" className="stroke-white/20" strokeWidth="12" fill="none" />
                      <circle cx="64" cy="64" r="56" className="stroke-white drop-shadow-md" strokeWidth="12" fill="none" strokeDasharray="351.85" strokeDashoffset={351.85 - (351.85 * report.readiness_score) / 100} style={{ transition: "stroke-dashoffset 1s ease-in-out" }} />
                    </svg>
                    <div className="absolute flex flex-col items-center">
                      <span className="text-4xl font-black">{report.readiness_score}</span>
                      <span className="text-[10px] uppercase font-bold tracking-widest opacity-80">Score</span>
                    </div>
                  </div>
                  <div>
                    <h2 className="text-2xl font-black mb-2">{report.company_name} Readiness</h2>
                    <p className="text-white/90 text-sm">
                      Based on your latest uploaded resume, your profile has a {report.readiness_score}% alignment for the <strong className="text-white">{report.target_role}</strong> role at {report.company_name}.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-5 rounded-2xl bg-card border border-border/10 shadow-sm">
                  <h3 className="font-bold mb-3 flex items-center gap-2">
                    <AlertTriangle className="h-4 w-4 text-warning" /> Missing Technical Skills
                  </h3>
                  {report.missing_skills?.technical?.length > 0 ? (
                    <ul className="space-y-2">
                      {report.missing_skills.technical.map((s: string, i: number) => (
                        <li key={i} className="text-sm flex items-center gap-2">
                          <XCircle className="h-4 w-4 text-destructive shrink-0" /> <span className="capitalize">{s}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-sm text-success flex items-center gap-2"><CheckCircle className="h-4 w-4" /> Strong technical alignment!</p>
                  )}
                </div>

                <div className="p-5 rounded-2xl bg-card border border-border/10 shadow-sm">
                  <h3 className="font-bold mb-3 flex items-center gap-2">
                    <AlertTriangle className="h-4 w-4 text-warning" /> Missing Project Experience
                  </h3>
                  {report.missing_skills?.projects?.length > 0 ? (
                    <ul className="space-y-2">
                      {report.missing_skills.projects.map((s: string, i: number) => (
                        <li key={i} className="text-sm flex items-center gap-2">
                          <XCircle className="h-4 w-4 text-destructive shrink-0" /> Projects using <span className="capitalize">{s}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-sm text-success flex items-center gap-2"><CheckCircle className="h-4 w-4" /> Project portfolio looks good!</p>
                  )}
                </div>
              </div>
              ...
              <div className="p-6 rounded-3xl bg-card border border-border/10 shadow-sm">
                <h3 className="font-bold text-lg mb-4 flex items-center gap-2">
                  <Sparkles className="h-5 w-5 text-primary" /> Personalized Action Plan
                </h3>
                <div className="space-y-3">
                  ...
                  {(!report.recommendations || report.recommendations.length === 0) && (
                    <p className="text-sm text-muted-foreground">You are highly aligned with this role! Start applying.</p>
                  )}
                </div>
              </div>
            ...
            <div className="h-full min-h-[400px] flex flex-col items-center justify-center p-8 rounded-3xl bg-card border border-dashed border-border/40 text-center">
              ...
              <h3 className="text-lg font-bold text-foreground">Select your target</h3>
              <p className="text-sm text-muted-foreground max-w-sm mt-2">
                Choose a company and a role to see how your current resume stacks up against their hiring bar.
              </p>
            </div>
          )}
        </div>
