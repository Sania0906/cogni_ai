...
import { MessageSquare, Code, Users, BookOpen, Sparkles, Building, Briefcase, ArrowRight, CheckCircle2, ChevronRight, XCircle } from "lucide-react";
...
type InterviewState = "setup" | "generating" | "active" | "evaluating" | "results";
...
function InterviewSimulator() {
  ...
  const handleSubmitInterview = async () => {
    ...
    try {
      const res = await api.evaluateInterview({ sessionId, answers: formattedAnswers });
      setResults(res);
      setState("results");
    ...
    <AppShell>
        ...
        <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4">
          <div className="rounded-3xl p-6 bg-gradient-primary text-white shadow-glow mb-5">
            ...
            <p className="text-sm text-white/80">
              We generate tailored interview questions by cross-analyzing your uploaded resume against the target company and role.
            </p>
          </div>

          <form onSubmit={handleGenerate} className="bg-card shadow-card p-5 rounded-3xl space-y-4 border border-border/10">
            <h3 className="text-lg font-bold text-card-foreground">Configure Simulation</h3>
      ...
      {state === "generating" && (
        <div className="flex flex-col items-center justify-center h-64 space-y-4 animate-in fade-in">
          ...
          <p className="text-sm font-bold text-muted-foreground">Synthesizing context-aware questions...</p>
        </div>
      )}
      ...
      {state === "evaluating" && (
        <div className="flex flex-col items-center justify-center h-64 space-y-4 animate-in fade-in">
          <div className="h-12 w-12 rounded-full border-4 border-primary border-t-transparent animate-spin" />
          <p className="text-sm font-bold text-muted-foreground">AI is evaluating your answers...</p>
        </div>
      )}
      ...
      {state === "results" && results && (
        <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4">
          <div className="rounded-3xl p-6 bg-card border border-border/10 shadow-card text-center">
            <h2 className="text-xl font-bold text-card-foreground mb-4">Interview Results</h2>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-2xl bg-gradient-primary text-white shadow-glow">
                <p className="text-xs opacity-80 font-semibold mb-1">Quality Score</p>
                <p className="text-3xl font-extrabold">{results.overall_quality_score}%</p>
              </div>
              <div className="p-4 rounded-2xl bg-gradient-pink text-white shadow-glow">
                <p className="text-xs opacity-80 font-semibold mb-1">Confidence Score</p>
                <p className="text-3xl font-extrabold">{results.overall_confidence_score}%</p>
              </div>
            </div>
            <button 
          ...
          <div className="space-y-4">
            {questions.map((q, idx) => {
              const evalData = results.evaluations.find((e: any) => e.question_id === q.id);
              return (
                <div key={q.id} className="p-5 rounded-3xl bg-card border border-border/10 shadow-card">
                  ...
                  <p className="text-sm font-bold text-card-foreground mb-3">{q.question_text}</p>
                  
                  <div className="bg-muted/30 p-3 rounded-xl mb-4 border border-border/40">
                    <p className="text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider">Your Answer</p>
                    <p className="text-sm font-medium text-card-foreground/80 italic">"{answers[q.id] || "No answer provided."}"</p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    <div className="flex items-center justify-between p-2 rounded-lg bg-card border border-border/40">
                      ...
                      <span className={`text-xs font-extrabold ${evalData?.quality_score >= 70 ? 'text-success' : 'text-destructive'}`}>
                        {evalData?.quality_score}%
                      </span>
                    </div>
                    <div className="flex items-center justify-between p-2 rounded-lg bg-card border border-border/40">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase">Confidence</span>
                      <span className={`text-xs font-extrabold ${evalData?.confidence_score >= 70 ? 'text-success' : 'text-destructive'}`}>
                  ...
                  <div className="p-3 rounded-xl bg-primary/5 border border-primary/10">
                    ...
                    <p className="text-sm font-medium text-card-foreground">{evalData?.improvement_suggestion}</p>
                  </div>
                </div>
              );
            })}
