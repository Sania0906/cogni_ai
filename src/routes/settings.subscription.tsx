...
const plans = [
  {
    ...
    price: "$0",
    period: "/mo",
    feats: ["3 skill assessments", "Basic AI assistant", "Community support"],
    current: false,
    accent: "bg-card",
  },
  {
      ...
      <div className="space-y-4">
        {plans.map((p) => {
          ...
          return (
            <div
              key={p.name}
              className={`rounded-3xl p-6 ${p.accent} ${isPro ? "text-white shadow-glow" : "shadow-card"}`}
            >
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <p
                      className={`text-xs uppercase tracking-wider ${isPro ? "text-white/80" : "text-muted-foreground"}`}
                    >
                      {p.name}
                    </p>
                  </div>
                  <p className="text-3xl font-bold mt-2">
                    {p.price}
                    <span
                      className={`text-base ${isPro ? "text-white/70" : "text-muted-foreground"}`}
                    >
                      {p.period}
                    </span>
                  </p>
              ...
              <ul className="mt-4 space-y-2">
                {p.feats.map((f) => (
                  <li key={f} className="flex items-center gap-2 text-sm">
                    <Check
                      className={`h-4 w-4 ${isPro ? "text-white" : "text-primary"}`}
                    />
                    {f}
                  </li>
                ))}
