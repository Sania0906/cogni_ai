...
function CareerSuccess() {
  ...
  useEffect(() => {
    api
      ...
      .catch((err) => {
        setError(
          err.message || "Failed to retrieve Career Success Probability",
        );
      });
  }, []);
      ...
      <AppShell>
        ...
        <div className="rounded-3xl p-6 bg-card border border-destructive/20 text-center space-y-4 max-w-sm mx-auto mt-10">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            ...
            Skill test) to unlock your Career Success Probability!
          </p>
        </div>
      </AppShell>
    );
    ...
    <AppShell>
      ...
      <div className="p-6 rounded-3xl bg-card shadow-card flex flex-col items-center mb-6">
        ...
        <p className="text-xs text-success font-semibold mt-1">
          Growth Outlook: {data.growthOutlook}
        </p>
      </div>

      {/* Breakdown List */}
      <h3 className="text-base font-bold mb-3 flex items-center gap-1.5">
        <ShieldCheck className="h-5 w-5 text-primary" /> Core Factor Evaluation
      </h3>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  High synergy with current skill set
                </p>
              </div>
            </div>
          ))}
