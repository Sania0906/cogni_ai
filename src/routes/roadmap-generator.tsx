...
import { Compass, CheckCircle2, Circle, Sparkles, Building, Briefcase, RefreshCw, Trophy } from "lucide-react";
    ...
    <AppShell>
      ...
      {state === "generating" && (
        <div className="flex flex-col items-center justify-center h-64 space-y-4 animate-in fade-in">
          ...
          <p className="text-sm font-bold text-muted-foreground">Mapping your personalized journey...</p>
        </div>
      )}
        ...
        <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4">
          <div className="rounded-3xl p-6 bg-gradient-primary text-white shadow-glow mb-5 relative overflow-hidden">
            ...
            <p className="text-sm text-white/80">
              We'll analyze your latest resume, identify missing skills, and generate a week-by-week curriculum to get you hired.
            </p>
          </div>

          <form onSubmit={handleGenerate} className="bg-card shadow-card p-5 rounded-3xl space-y-4 border border-border/10">
          ...
          <div className="rounded-3xl p-6 bg-gradient-primary text-white shadow-glow relative overflow-hidden flex flex-col">
            ...
            <div className="flex justify-between items-start mb-4 z-10">
              <div>
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-xs uppercase tracking-wider font-semibold text-white/80">Target Milestone</span>
                </div>
                <h2 className="text-2xl font-bold">{roadmap.target_role} @ {roadmap.target_company}</h2>
              </div>
              <button 
            ...
            <div className="bg-white/10 p-4 rounded-2xl border border-white/20 z-10">
              <div className="flex justify-between items-center mb-2">
                <span className="text-xs font-bold text-white/90">Overall Progress</span>
                <span className="text-sm font-extrabold">{roadmap.overall_progress}%</span>
              </div>
            ...
            {milestones.map((node, idx) => {
              ...
              return (
                <div key={node.id} className="relative">
                  ...
                  <div className={`p-5 rounded-2xl shadow-card border transition-colors ${isCompleted ? "bg-success/5 border-success/30" : "bg-card border-border/20"}`}>
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Week {node.week_number}</span>
                      {isCompleted && <span className="text-[10px] font-bold text-success uppercase bg-success/10 px-2 py-0.5 rounded-md">Completed</span>}
                    </div>
                    <h4 className={`font-bold text-base leading-tight mb-2 ${isCompleted ? 'line-through text-muted-foreground' : 'text-card-foreground'}`}>
                      {node.title}
                    </h4>
                    ...
                    <div className="space-y-3">
                      {node.technologies && node.technologies.length > 0 && (
                        <div>
                          <span className="text-[10px] font-bold text-muted-foreground uppercase block mb-1">Technologies</span>
                          <div className="flex gap-1.5 flex-wrap">
                            {node.technologies.map((t: string) => (
                              <span key={t} className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-primary/10 text-primary">
                                {t}
                              </span>
                            ))}
                          </div>
                      ...
                      {node.practice_projects && node.practice_projects.length > 0 && (
                        <div>
                          <span className="text-[10px] font-bold text-muted-foreground uppercase block mb-1">Practice Projects</span>
                          <div className="flex flex-col gap-1">
                            {node.practice_projects.map((p: string) => (
                              <span key={p} className="text-xs font-semibold text-card-foreground flex items-center gap-1.5 before:content-[''] before:h-1 before:w-1 before:rounded-full before:bg-muted-foreground">
                                {p}
                              </span>
                            ))}
                          </div>
                      ...
                      {node.certifications && node.certifications.length > 0 && (
                        <div>
                          <span className="text-[10px] font-bold text-muted-foreground uppercase block mb-1">Certifications</span>
                          <div className="flex flex-col gap-1">
                            {node.certifications.map((c: string) => (
                              <span key={c} className="text-xs font-semibold text-pink-500 flex items-center gap-1.5">
                                🏆 {c}
                              </span>
                            ))}
                          </div>
