...
import { Search, Filter, Bookmark, BookmarkCheck, Download, Users, SlidersHorizontal, ArrowRight, Activity, ShieldCheck, X } from "lucide-react";
...
function RecruiterDashboard() {
  ...
  const handleToggleShortlist = async (candidateId: string) => {
    try {
      ...
      setCandidates(prev => prev.map(c => c.id === candidateId ? { ...c, is_shortlisted: res.shortlisted } : c));
  ...
  const filteredCandidates = candidates.filter(c => 
    ...
    (c.current_job_title && c.current_job_title.toLowerCase().includes(searchQuery.toLowerCase()))
  );
    ...
    <AdminShell title="Recruiter Dashboard">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center mb-6">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <button 
            onClick={() => { if (selectedForCompare.length > 1) setShowCompareModal(true); else toast.error("Select at least 2 candidates to compare."); }}
      ...
      <div className="bg-card shadow-card rounded-2xl p-5 mb-6 border border-border/50 flex flex-col md:flex-row gap-5 items-center">
        ...
        <div className="flex-1 w-full flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1.5 block">Target Skills</label>
            <input 
              type="text"
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-muted-foreground font-medium">No candidates match your filters.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredCandidates.map(c => {
            const isComparing = selectedForCompare.some(p => p.id === c.id);
            return (
              <div key={c.id} className={`bg-card shadow-card rounded-2xl p-5 border-2 transition-all ${isComparing ? "border-primary" : "border-transparent"} hover:-translate-y-1`}>
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-card-foreground leading-tight">{c.name}</h3>
                      <p className="text-xs text-muted-foreground">{c.current_job_title || "Software Engineer"}</p>
                    </div>
                  </div>
                  <button 
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-muted/30 rounded-xl p-3 border border-border/40">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1">ATS Score</p>
                    <div className="flex items-end gap-1">
                      <span className="text-xl font-extrabold text-card-foreground leading-none">{c.ats_score}</span>
                      <span className="text-xs text-muted-foreground font-medium pb-0.5">/ 100</span>
                    </div>
                  </div>
                  <div className="bg-muted/30 rounded-xl p-3 border border-border/40">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1">Experience</p>
                    <div className="flex items-end gap-1">
                      <span className="text-xl font-extrabold text-card-foreground leading-none">{c.experience_years}</span>
                      <span className="text-xs text-muted-foreground font-medium pb-0.5">Years</span>
                    </div>
                  </div>
                </div>

                <div className="mb-5">
                  <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-2">Top Skills</p>
                  <div className="flex flex-wrap gap-1.5">
                    {c.skills.slice(0, 5).map((s: string) => (
                      <span key={s} className="px-2 py-1 rounded-md bg-primary/10 text-primary text-[10px] font-bold">
                        {s}
                      </span>
                    ))}
                    {c.skills.length > 5 && (
            ...
            <div className="px-6 py-5 border-b border-border flex justify-between items-center bg-muted/20">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Evaluating {selectedForCompare.length} candidates side-by-side</p>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-auto p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {selectedForCompare.map(c => (
                  <div key={c.id} className="border-2 border-border/50 rounded-2xl p-5 flex flex-col relative overflow-hidden">
                    {c.ats_score > 85 && (
                    ...
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{c.current_job_title}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6 flex-1">
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-xs font-bold uppercase text-muted-foreground">ATS Score</span>
                          <span className="text-lg font-black text-primary">{c.ats_score}%</span>
                        </div>
                        <div className="w-full bg-muted rounded-full h-2">
                          <div className="bg-primary h-2 rounded-full" style={{ width: `${c.ats_score}%` }} />
                        </div>
                      </div>
                      
                      <div className="border-t border-border/50 pt-4">
                        <span className="text-xs font-bold uppercase text-muted-foreground mb-3 block">Core Metrics</span>
                        <div className="flex justify-between items-center bg-muted/30 p-3 rounded-xl mb-2">
                          <span className="text-sm font-medium">Experience</span>
                          <span className="text-sm font-bold">{c.experience_years} Years</span>
                        </div>
                        <div className="flex justify-between items-center bg-muted/30 p-3 rounded-xl">
                          ...
                          <span className="text-sm font-bold">{c.is_shortlisted ? "Yes" : "No"}</span>
                        </div>
                      </div>

                      <div className="border-t border-border/50 pt-4">
                        <span className="text-xs font-bold uppercase text-muted-foreground mb-3 block">Skills Match</span>
                        <div className="flex flex-wrap gap-2">
                          {c.skills.slice(0, 8).map((s: string) => (
                            <span key={s} className="px-2.5 py-1 rounded-lg bg-card border border-border shadow-sm text-xs font-semibold">
                              {s}
                            </span>
                          ))}
                          {c.skills.length > 8 && (
                    ...
                    >
                      {c.is_shortlisted ? <BookmarkCheck className="h-4 w-4" /> : <Bookmark className="h-4 w-4" />}
