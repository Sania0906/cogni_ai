    ...
    <AppShell>
      ...
      <div className="rounded-3xl p-6 bg-card shadow-card">
        ...
        <div className="flex flex-wrap gap-3 mt-4 text-sm">
          ...
          <span className="flex items-center gap-1 text-muted-foreground">
            <DollarSign className="h-4 w-4" /> $140k–180k
          </span>
        </div>
      </div>
      ...
      <div className="rounded-2xl p-5 bg-gradient-primary text-white shadow-glow mt-4">
        ...
        <p className="text-xs text-white/80 mt-1">
          Excellent fit based on your skills
        </p>
      </div>

      <h3 className="text-lg font-bold mt-6 mb-3">About the role</h3>
      <p className="text-sm text-muted-foreground leading-relaxed">
        Build and ship ML systems at scale. Work with state-of-the-art models,
        mentor engineers, and shape product strategy.
      </p>

      <h3 className="text-lg font-bold mt-6 mb-3">Required Skills</h3>
      <div className="flex flex-wrap gap-2">
        {["Python", "PyTorch", "MLOps", "AWS", "Distributed Systems"].map(
          (s) => (
            <span
              key={s}
              className="px-3 py-1.5 rounded-full bg-accent text-accent-foreground text-xs font-semibold"
            >
              {s}
            </span>
          ),
      ...
      <div className="grid grid-cols-2 gap-3 mt-6">
        <Link
          to="/saved-jobs"
          className="py-4 text-center rounded-2xl bg-card shadow-card font-semibold"
        >
          Save
        </Link>
        <Link
          to="/applied-jobs"
          className="py-4 text-center rounded-2xl bg-gradient-primary text-white font-bold shadow-glow"
        >
          Apply Now
        </Link>
      </div>
