  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      for (const course of courses) {
        const keywords = (course.keywords || []).map((k: string) =>
          k.toLowerCase(),
        );
        const isMissingKeyword = keywords.some((kw: string) =>
          missingKeywords.some((mk: string) => mk.includes(kw)),
        );
        ...
        if (isMissingKeyword) {
          recommendations.push({
            ...
            reason:
              "Identified as a missing competency in your latest resume ATS scan.",
          });
        }
      }
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch similar profiles" });
    }
  },
);
