...
const DEFAULT_JOBS = [
  {
    ...
    salary: "$140k - $180k",
    match: 95,
    description: "Lead machine learning models deployment.",
  ...
  },
  {
    ...
    salary: "$130k - $170k",
    match: 91,
    description: "Deploy deep learning computer vision solutions.",
  ...
  },
  {
    ...
    salary: "$160k - $220k",
    match: 88,
    description: "Pioneering LLM research projects.",
    requirements: ["Deep Learning", "Transformers", "Math"],
  },
  {
    id: "job_4",
    ...
    salary: "$120k - $155k",
    match: 72,
    description: "Establish scalable pipeline orchestrations.",
    requirements: ["Docker", "MLflow", "FastAPI"],
  },
  {
    id: "job_web_1",
  ...
  },
  {
    ...
    salary: "$110k - $140k",
    match: 82,
    description: "Audit security protocols and networks.",
    requirements: ["Network Security", "Linux", "Cryptography"],
  },
];
...
router.get("/", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    let { data, error } = await supabaseAdmin.from("jobs").select("*");
    ...
    return res
      .status(500)
      .json({ message: err.message || "Failed to retrieve jobs catalog" });
  }
  return res.json([]);
});
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: err.message || "Failed to fetch applied jobs" });
    }
  },
);
    ...
    if (
      interests.some(
        (i: string) =>
          ...
          i.toLowerCase().includes("stack") ||
          i.toLowerCase().includes("javascript") ||
      ...
      interests.some(
        (i: string) =>
          ...
          i.toLowerCase().includes("machine"),
      )
    ) {
      role = "AI Engineer";
      average = 155000;
