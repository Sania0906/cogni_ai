  ...
  try {
    ...
    if (!resumeUploaded) {
      return {
        locked: true,
        message:
          "AI Career insights are locked. Please upload your resume first to unlock AI Analysis.",
        checklist: {
          profile: true,
          assessment: true,
          resume: false,
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const progScore =
      userAssessments?.find((a) => a.category.toLowerCase().includes("prog"))
        ?.score || 80;
    const dbScore =
      userAssessments?.find(
        (a) =>
          a.category.toLowerCase().includes("db") ||
    ...
    const aiScore =
      userAssessments?.find(
        (a) =>
          ...
          a.category.toLowerCase().includes("ml"),
      )?.score || 70;
    const cloudScore =
      userAssessments?.find(
        (a) =>
          a.category.toLowerCase().includes("cloud") ||
          a.category.toLowerCase().includes("sys"),
      )?.score || 70;
    const webScore =
      userAssessments?.find(
        (a) =>
          a.category.toLowerCase().includes("web") ||
          a.category.toLowerCase().includes("front"),
      )?.score || 75;
    const aptScore =
      userAssessments?.find(
        (a) =>
          a.category.toLowerCase().includes("apt") ||
          a.category.toLowerCase().includes("logic"),
      )?.score || 80;

    // Dynamic Archetype
    let archetype = "The Algorithmic Architect";
    let tagline =
      "Highly analytical and detail-oriented, with a natural talent for abstract system designing.";

    if (webScore > progScore && webScore > aiScore) {
      archetype = "The Interface Virtuoso";
      tagline =
        "Visually creative and systems-savvy, bringing user interfaces to life with elegant designs.";
    } else if (aiScore > progScore && aiScore > webScore) {
      archetype = "The Cognitive Scientist";
      tagline =
        "Intrigued by neural systems, pattern matching, and predicting future trends from complex data models.";
    } else if (dbScore > progScore && dbScore > webScore) {
      archetype = "The Schema Strategist";
      tagline =
        "Optimizing data stores, pipelines, and indexes for speed and reliable data processing.";
    }

    const maxTech = Math.max(progScore, dbScore, aiScore, webScore, cloudScore);
    const leadershipScore = userProfile?.cgpa
      ? Math.min(100, Math.round(Number(userProfile.cgpa) * 10) + 15)
      : 70;
    ...
    try {
      await supabaseAdmin.from("career_dna").upsert({
        user_id: userId,
        archetype,
        tagline,
        dimensions,
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const avgAssessment =
      userAssessments && userAssessments.length > 0
        ? Math.round(
            userAssessments.reduce((sum, a) => sum + a.score, 0) /
              userAssessments.length,
          )
        : 80;

    const atsScore = userResume?.ats_score || 75;
    const cgpaFactor = userProfile?.cgpa
      ? Math.round(parseFloat(userProfile.cgpa) * 10)
      : 80;

    const probabilityScore = Math.round(
      (avgAssessment + atsScore + cgpaFactor) / 3,
    );

    const interests = (userProfile?.interests || []).map((i: string) =>
      i.toLowerCase().trim(),
    );
    const hasWeb = interests.some(
      (t: string) =>
        t.includes("web") ||
        t.includes("stack") ||
    ...
    const hasCloud = interests.some(
      (t: string) =>
        t.includes("cloud") || t.includes("aws") || t.includes("devops"),
    );
    const hasSecurity = interests.some(
      (t: string) => t.includes("security") || t.includes("cyber"),
    );
    ...
    let alternativeRoles = [
      {
        ...
        probability: Math.max(40, probabilityScore - 4),
      },
      {
        role: "Data Solutions Architect",
        probability: Math.max(40, probabilityScore - 8),
      },
      {
        role: "Quantitative Analyst",
        probability: Math.max(40, probabilityScore - 15),
      },
    ];
    ...
    if (hasWeb) {
      ...
      alternativeRoles = [
        ...
        {
          role: "Security Auditor / Architect",
          probability: Math.max(40, probabilityScore - 15),
        },
      ];
    }
    ...
    const result = {
      ...
      breakdown: [
        {
          name: "Technical Skill Fit",
          rating: avgAssessment,
          detail: `Your assessment average across skill categories is ${avgAssessment}%.`,
        },
        {
          name: "Resume Impact (ATS)",
          rating: atsScore,
          detail: `Your uploaded resume matches ATS benchmarks at ${atsScore}%.`,
        },
        {
          name: "Academic Standing",
          rating: cgpaFactor,
          detail: `Your CGPA of ${userProfile?.cgpa || "N/A"} places you in the quantitative top tier.`,
        },
        {
          name: "Market Liquidity / Openings",
          rating: 90,
          detail:
            "Strong market demand indicates rapid hiring speed (average 22 days to offer).",
        },
      ],
...
router.get("/industry-demand", async (req, res) => {
  try {
    ...
    const marketDrivers = Array.from(
      new Set(categories.flatMap((item) => item.market_drivers || [])),
    );
    ...
    return res
      .status(500)
      .json({ message: "Failed to retrieve market demands" });
  }
});
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      let missingSkills = [];
      if (
        targetKeywords.includes("frontend") ||
        targetKeywords.includes("react")
      ) {
        missingSkills = [
          "Next.js",
          "State Management",
        ...
        targetKeywords.includes("data") ||
        targetKeywords.includes("ml")
      ) {
        missingSkills = ["Deep Learning", "MLOps", "Model Deployment"].filter(
          (s) =>
            !skills.some((us: string) =>
              us.toLowerCase().includes(s.toLowerCase()),
            ),
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const overallScore = Math.round(
      (avgAssessment + atsScore + certScore + cgpaFactor) / 4,
    );
    ...
    const components = [
      {
        ...
        score: avgAssessment,
        status: avgAssessment >= 85 ? "Excellent" : "Good",
      },
      {
        label: "Certifications & Credentials",
        score: certScore,
        status: certScore >= 80 ? "Excellent" : "Needs Work",
      },
      {
        label: "Resume Completeness & Impact",
        score: atsScore,
    ...
    const feedback = [
      ...
      certsCount < 3
        ? "Boost score by completing remaining assignments and obtaining certification badges."
        : "Great job on earning certifications.",
      "Consider publishing 1-2 research notebooks or portfolio websites.",
    ];

    try {
      await supabaseAdmin.from("employability_scores").upsert({
        user_id: userId,
...
function parseResumeText(text: string): {
  ...
  const portfolioMatch = text.match(
    /(?:https?:\/\/)?(?:www\.)?([a-zA-Z0-9-]+\.(?:dev|me|io|com)(?:\/[a-zA-Z0-9_-]+)*)/i,
  );
  const portfolio =
    portfolioMatch &&
  ...
  const degreeRegex =
    /(?:Bachelor|Master|B\.Tech|M\.Tech|B\.S\.|M\.S\.|B\.Sc|M\.Sc|Ph\.D|PhD|Associate|Degree)\s+(?:of|in)?\s*([A-Za-z\s&]{3,40})/i;
  const degreeMatch = text.match(degreeRegex);
  if (degreeMatch) degree = degreeMatch[0].trim();

  const collegeRegex =
    /(?:University|College|Institute|School|Academy|IIT|NIT)\s+of\s+[A-Za-z\s]+|[A-Za-z\s]+\s+(?:University|College|Institute|School|Academy|IIT|NIT)/i;
  const collegeMatch = text.match(collegeRegex);
  if (collegeMatch) college = collegeMatch[0].trim();

  const cgpaRegex =
    /\b(cgpa|gpa|score|grade)\b\s*(?:of|is|:)?\s*([0-9]\.[0-9]{1,2}(?:\s*\/\s*(?:10|4))?)/i;
  const cgpaMatch = text.match(cgpaRegex);
  if (cgpaMatch) {
    cgpa = cgpaMatch[2].split("/")[0].trim();
  } else {
    const floatMatch = text.match(
      /\b([2-3]\.[0-9]{1,2}\s*\/\s*4|[6-9]\.[0-9]{1,2}\s*\/\s*10)\b/,
    );
  ...
  const languages = languageList.filter((l) =>
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    if (req.file) {
      try {
        ...
        return res
          ...
          .json({
            message:
              "Failed to parse resume file. Ensure it is a valid PDF or text document.",
          });
      }
    }
    ...
    if (!resumeText || !resumeText.trim()) {
      return res
        .status(400)
        .json({
          message: "Resume content is required (either as text or file upload)",
        });
    }
    ...
    const keywords = [
      ...
      {
        word: "Deep Learning",
        regex: /\bdeep\s+learning\b|\bdl\b/,
        suggestion:
          "Add details of PyTorch projects or CNN/RNN implementation.",
      },
      {
        word: "MLOps",
        regex: /\bmlops\b|\bmodel\s+deployment\b/,
        suggestion: "Mention MLOps toolings like MLflow, Docker or kubeflow.",
      },
      { word: "SQL", regex: /\bsql\b|\bdatabases\b/ },
      {
        ...
        regex: /\bpytorch\b/,
        suggestion:
          "Explicitly highlight PyTorch library experience if applicable.",
      },
      {
        word: "Docker",
        regex: /\bdocker\b|\bcontainers\b/,
        suggestion: "Mention containerization skills using Docker.",
      },
    ];
      ...
      return {
        ...
        ...(found
          ...
          : {
              suggestion:
                kw.suggestion ||
                `Add context around your experience with ${kw.word}.`,
            }),
      };
    });
    ...
    if (score < 75) {
      improvements.push(
        "Incorporate more active verbs and quantify accomplishments (e.g. 'Improved latency by 20%').",
      );
    }
    if (
      !lowercaseText.includes("pytorch") &&
      !lowercaseText.includes("tensorflow")
    ) {
      improvements.push(
        "Detail your experience with deep learning frameworks (PyTorch or TensorFlow) in project bullet points.",
      );
    }
    if (
      !lowercaseText.includes("docker") &&
      !lowercaseText.includes("mlflow")
    ) {
      improvements.push(
        "Showcase model packaging and deployment tools (Docker, Kubernetes, MLflow) to strengthen MLOps alignment.",
      );
    }
    if (improvements.length === 0) {
      improvements.push(
        "Excellent work. Consider tailoring resume bullet points specifically to the company's tech stack.",
      );
    }
    ...
    try {
      ...
      console.warn(
        "Failed to fetch user profile during resume optimization:",
        err,
      );
    }
    ...
    const certificationsMatched = parsedResume.certifications.map(
      (c) => c.name,
    );
    ...
    if (resumeText.length < 500) {
      formattingIssues.push(
        "Resume length is very short. Expand descriptions to showcase accomplishments.",
      );
    }
    if (!parsedResume.email) {
      formattingIssues.push(
        "Contact email not explicitly detected in formatting scan.",
      );
    }
    if (!parsedResume.education.degree) {
      formattingIssues.push(
        "Academic or education headers are not clearly labeled.",
      );
    }
    ...
    const strengths = keywordMatch
      ...
      .map((k) => k.word);
    const weaknesses = [
      ...keywordMatch.filter((k) => k.status === "missing").map((k) => k.word),
      ...formattingIssues,
    ];
    ...
    try {
      ...
      const { error: resumeError } = await supabaseAdmin
        ...
        .upsert({
          ...
          parsed_text: JSON.stringify({
            // Store extended dynamically extracted details in parsed_text JSON
      ...
      if (parsedResume.name && parsedResume.name !== "Professional User")
        profileUpdate.name = parsedResume.name;
      if (parsedResume.email) profileUpdate.email = parsedResume.email;
      if (parsedResume.linkedin)
        profileUpdate.linkedin_url = parsedResume.linkedin;
      if (parsedResume.github) profileUpdate.github_url = parsedResume.github;
      if (parsedResume.education.degree)
        profileUpdate.degree = parsedResume.education.degree;
      if (parsedResume.education.college)
        profileUpdate.college = parsedResume.education.college;
      if (parsedResume.education.cgpa)
        profileUpdate.cgpa = parsedResume.education.cgpa;
      ...
      await supabaseAdmin.from("academic_details").upsert({
        user_id: userId,
        degree: parsedResume.education.degree,
        college: parsedResume.education.college,
        cgpa: parseFloat(parsedResume.education.cgpa),
      ...
      const isWebDev =
        (userProfile?.interests || []).some(
          (i: string) =>
            i.toLowerCase().includes("web") ||
            i.toLowerCase().includes("react"),
        ) ||
        extractedSkills.includes("React") ||
        extractedSkills.includes("JavaScript");
      const isCloudOps =
        (userProfile?.interests || []).some(
          (i: string) =>
            i.toLowerCase().includes("cloud") ||
            i.toLowerCase().includes("devops"),
        ) ||
        extractedSkills.includes("AWS") ||
        extractedSkills.includes("Docker");
      ...
      const dimensions = [
        ...
        { subject: "Creative Problem Solving", val: 80, angle: 60 },
        {
          ...
          val: isCloudOps ? 90 : 75,
          angle: 120,
        },
        { subject: "Adaptability", val: 80, angle: 180 },
        {
          subject: "Technical Expertise",
          val: Math.min(100, 50 + extractedSkills.length * 5),
          angle: 240,
        },
      ...
      const recommendedEnvironments = isWebDev
        ? [
            "Fast-growing frontend product startups",
            "Digital design and development agencies",
          ]
        : isCloudOps
          ? [
              "Distributed systems and platform infrastructure teams",
              "DevOps/SRE scaling divisions",
            ]
          : [
              "Research labs / R&D Quantitative divisions",
              "Fast scaling product startups",
      ...
      if (dnaError) {
        console.error(
          "[Database Error] Failed to upsert Career DNA:",
          dnaError,
        );
        throw new Error(
          `Failed to save Career DNA details: ${dnaError.message}`,
        );
      }
      ...
      const certScore = Math.min(
        100,
        50 + parsedResume.certifications.length * 15,
      );
      ...
      const feedback = [
        ...
        parsedResume.certifications.length < 3
          ? "Boost score by completing remaining assignments and obtaining certification badges."
          : "Great job on earning certifications.",
        "Consider publishing 1-2 research notebooks or portfolio websites.",
      ];
      ...
      if (empError) {
        console.error(
          "[Database Error] Failed to upsert Employability Score:",
          empError,
        );
        throw new Error(
          `Failed to save Employability Score details: ${empError.message}`,
        );
      }
    } catch (dbErr: any) {
      ...
      return res
        .status(500)
        .json({ message: dbErr.message || "Failed to save data to Supabase" });
    }
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: err.message || "Failed to retrieve ATS report" });
    }
  },
);
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const { data: profile } = await supabaseAdmin
      ...
      .eq("id", userId)
      .maybeSingle();
    const interests = (profile?.interests || []).map((i: string) =>
      i.toLowerCase().trim(),
    );
    ...
    let roles = [
      {
        ...
        growth: 120,
        emerging: true,
        notes: "Requires compliance and data ethics foundation.",
      },
      {
        name: "Prompt Engineer / Agent Architect",
        risk: 55,
        growth: 45,
        emerging: true,
        notes: "Transitioning into general engineering skillset.",
      },
      {
        name: "MLOps Platform Architect",
        risk: 8,
        growth: 145,
        emerging: true,
        notes: "Extremely high market demand for infrastructure scaling.",
      },
      {
        name: "Model Quantization & Local Inference Engineer",
        risk: 10,
        growth: 180,
        emerging: true,
        notes: "Focuses on running models on edge devices.",
      },
    ];
    ...
    if (
      ...
      roles = [
        {
          name: "AI-Augmented Frontend Developer",
          risk: 25,
          growth: 80,
          emerging: true,
          notes: "Integrates agentic UI patterns into core frameworks.",
        },
        {
          name: "Web3 Decentralized Systems Engineer",
          risk: 40,
          growth: 35,
          emerging: false,
          notes: "Steady adoption across fintech sectors.",
        },
        {
          name: "Dynamic Interface Architect",
          risk: 15,
          growth: 95,
          emerging: true,
          notes: "Designing real-time, LLM-generated UI structures.",
        },
        {
          name: "Full Stack MLOps Engineer",
          risk: 12,
          growth: 130,
          emerging: true,
          notes: "Bridges models and client interfaces.",
        },
      ];
    } else if (
      interests.some(
        (i: string) => i.includes("security") || i.includes("cyber"),
      )
    ) {
      roles = [
        {
          name: "AI Penetration Tester / Red Teamer",
          risk: 5,
          growth: 210,
          emerging: true,
          notes: "Focuses on prompt injection and model jailbreak defenses.",
        },
        {
          name: "Cryptographic Vulnerability Auditor",
          risk: 8,
          growth: 95,
          emerging: true,
          notes: "Post-quantum security systems testing.",
        },
        {
          name: "Autonomous Defense Analyst",
          risk: 12,
          growth: 160,
          emerging: true,
          notes: "Monitoring automated agent cyber attacks.",
        },
      ];
    }
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const cgpaScore =
      cgpa > 4.0
        ? Math.min(100, Math.round((cgpa / 10.0) * 100))
        : Math.min(100, Math.round((cgpa / 4.0) * 100));
    const atsScore = userResume?.ats_score || 75;
    ...
    const hasSkill = (skillName: string) => {
      ...
      if (
        matchingSkill &&
        (matchingSkill.progress >= 50 || matchingSkill.level !== "Beginner")
      )
        return true;
    ...
    const results = pathsDef.map((path) => {
      ...
      const matchPercentage = Math.min(
        ...
        Math.max(
          35,
          Math.round(skillsMatch * 0.5 + atsScore * 0.3 + cgpaScore * 0.2),
        ),
      );
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const skillsList =
      skills.map((s) => s.name).join(", ") || "No custom skills listed";
    const atsScore = atsReport?.score || resume?.ats_score || "not scanned yet";
    const improvements =
      atsReport?.recommendations || resume?.improvements || [];
    ...
    if (
      ...
      if (atsScore === "not scanned yet") {
        ...
        reply =
          `Hi ${name}, your current ATS score is ${atsScore}/100. Based on your parsed resume, here are the key improvements I recommend to increase readability:\n\n` +
          improvements
      ...
      msgLower.includes("employability") ||
      msgLower.includes("index") ||
      msgLower.includes("competitiveness")
    ) {
      ...
      msgLower.includes("dna") ||
      msgLower.includes("archetype") ||
      msgLower.includes("personality")
    ) {
      if (archetype === "not generated yet") {
        ...
        reply =
          `Hi ${name}, your AI Career DNA Archetype is **${archetype}** ("${tagline}"). Based on this profile, the environments most suited to your personality are:\n` +
      ...
      const missing = [];
      if (!skillsList.toLowerCase().includes("python"))
        missing.push("Python (Programming)");
      if (
        !skillsList.toLowerCase().includes("sql") &&
        !skillsList.toLowerCase().includes("database")
      )
        missing.push("SQL (Database)");
      if (
        !skillsList.toLowerCase().includes("machine") &&
        !skillsList.toLowerCase().includes("deep")
      )
        missing.push("Machine Learning/Deep Learning");
      if (
        !skillsList.toLowerCase().includes("docker") &&
        !skillsList.toLowerCase().includes("kubernetes")
      )
        missing.push("Docker & MLOps");

      reply = `Hi ${name}, analyzing your current skill profile (${skillsList || "none listed yet"}), you have strong foundations. `;
      if (missing.length > 0) {
        reply +=
          `To become fully competitive in target roles, you should prioritize building skills in:\n` +
      ...
      if (
        skillsList.toLowerCase().includes("react") ||
        skillsList.toLowerCase().includes("javascript")
      ) {
        target = "Full Stack Web Developer";
      } else if (
        skillsList.toLowerCase().includes("aws") ||
        skillsList.toLowerCase().includes("docker") ||
        skillsList.toLowerCase().includes("cloud")
      ) {
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch resume history." });
    }
  },
);
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (!history || history.length < 2) {
        return res.json({
          message: "Not enough resume versions to compare.",
          canCompare: false,
        });
      }
      ...
      const addedSkills = (latest.skills || []).filter(
        (s: string) => !(previous.skills || []).includes(s),
      );
      const removedSkills = (previous.skills || []).filter(
        (s: string) => !(latest.skills || []).includes(s),
      );
      const newProjects = (latest.projects || []).filter(
        (p: any) =>
          !(previous.projects || []).some((pp: any) => pp.title === p.title),
      );
      const newCertifications = (latest.certifications || []).filter(
        (c: any) =>
          !(previous.certifications || []).some((pp: any) =>
            typeof c === "string" ? pp === c : pp.name === c.name,
          ),
      );

      const atsImprovement =
        (latest.ats_score || 0) - (previous.ats_score || 0);

      // Dynamic Missing Sections Detection
      const missingSections = [];
      if (
        !latest.education ||
        latest.education.trim() === "" ||
        latest.education === " at  (CGPA: )"
      )
        missingSections.push("Education");
      if (!latest.projects || latest.projects.length === 0)
        missingSections.push("Projects");
      if (!latest.experience || latest.experience.length === 0)
        missingSections.push("Work Experience");
      if (!latest.certifications || latest.certifications.length === 0)
        missingSections.push("Certifications");
      if (!latest.skills || latest.skills.length === 0)
      ...
      return res
        .status(500)
        .json({ message: "Failed to compute resume comparison." });
    }
  },
);
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (resumeError) {
        console.error(
          "[Database Error] Failed to restore resume:",
          resumeError,
        );
        return res
          .status(500)
          .json({ message: "Failed to restore resume state." });
      }
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    if (!companyName || !targetRole) {
      return res
        .status(400)
        .json({ message: "Company Name and Target Role are required." });
    }
    ...
    try {
      ...
      if (!resumes || resumes.length === 0) {
        return res
          .status(404)
          .json({ message: "No resume found. Please upload a resume first." });
      }
      ...
      if (companyName.toLowerCase() === "microsoft")
        techReqs.push("C#", ".NET", "Azure");
      if (companyName.toLowerCase() === "amazon")
        techReqs.push("AWS", "DynamoDB");
      ...
      const requiredCerts =
        companyName.toLowerCase() === "amazon"
          ? ["AWS Certified Solutions Architect"]
          : companyName.toLowerCase() === "microsoft"
            ? ["Azure Fundamentals"]
            : [];

      // 3. Analyze Matches
      const userSkillsLower = userSkills.map((s: string) => s.toLowerCase());
      const missingTech = techReqs.filter(
        (r) => !userSkillsLower.includes(r.toLowerCase()),
      );

      // Check if user has projects mapped to these tech reqs
      const projectTech = userProjects
        .flatMap((p: any) => p.technologies || [])
        .map((t: string) => t.toLowerCase());
      const missingProjectExp = techReqs.filter(
        (r) => !projectTech.includes(r.toLowerCase()),
      );

      // Experience Check
      const missingSoft = requiredSoft.filter(
        (r) => !userSkillsLower.includes(r.toLowerCase()),
      );
      ...
      const techScore = Math.max(
        0,
        100 - missingTech.length * (100 / techReqs.length),
      );
      ...
      if (missingTech.length > 0) {
        recommendations.push(
          `Master these missing technologies: ${missingTech.slice(0, 3).join(", ")}`,
        );
      }
      if (missingProjectExp.length > 0) {
        recommendations.push(
          `Build projects showcasing: ${missingProjectExp.slice(0, 2).join(", ")}`,
        );
      }
      if (missingCerts.length > 0) {
        recommendations.push(
          `Consider pursuing certifications like ${missingCerts[0]} to stand out at ${companyName}.`,
        );
      }
      if (finalScore < 60) {
        recommendations.push(
          `Your profile needs significant alignment with ${companyName}'s requirements for a ${targetRole} role.`,
        );
      }
      ...
      if (insertError) {
        console.error(
          "[Database Error] Failed to save company readiness report:",
          insertError,
        );
        return res
          .status(500)
          .json({ message: "Analysis complete, but failed to save report." });
      }
      ...
      return res
        .status(500)
        .json({ message: "Failed to generate company readiness report." });
    }
  },
);
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (error && error.code !== "PGRST116") {
        console.error(
          "[Database Error] fetching latest company readiness:",
          error,
        );
        return res
          .status(500)
          .json({ message: "Failed to fetch company readiness report." });
      }

      if (!report) {
        return res
          .status(404)
          .json({ message: "No company readiness report found." });
      }
      ...
      return res
        .status(500)
        .json({ message: "Server error fetching company readiness report." });
    }
  },
);
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const softMatches = softSkillsKeywords.filter((k) =>
        rawText.includes(k),
      ).length;
      ...
      const leadMatches = leadershipKeywords.filter((k) =>
        rawText.includes(k),
      ).length;
      ...
      const commMatches = commKeywords.filter((k) =>
        rawText.includes(k),
      ).length;
      ...
      const probMatches = problemKeywords.filter((k) =>
        rawText.includes(k),
      ).length;
      const problem_solving_score = Math.min(100, 45 + probMatches * 10);

      const learning_ability = Math.min(
        100,
        50 + (latestResume.certifications?.length || 0) * 15,
      );

      // 2. Career Maturity Level
      const totalExpItems = userExperience.length;
      let maturity_level = "Entry Level";
      if (totalExpItems >= 2 && leadMatches > 0)
        maturity_level = "Mid-Level Professional";
      if (totalExpItems > 3 && leadMatches >= 2)
        maturity_level = "Senior Professional";
      if (leadMatches >= 4) maturity_level = "Executive / Staff Level";

      // 3. Career Personality
      let personality = "The Balanced Professional";
      if (technical_score > 85 && leadership_potential < 60)
        personality = "The Deep Technologist";
      if (leadership_potential > 80 && soft_skills_score > 80)
        personality = "The Visionary Leader";
      if (problem_solving_score > 85 && communication_score > 75)
        personality = "The Strategic Problem Solver";
      ...
      if (userSkills.length > 0)
        top_strengths.push(`Core competency in ${userSkills[0]}`);
      if (userExperience.length > 0)
        top_strengths.push("Proven track record in professional environments");

      const improvement_areas = dimensions
        .slice(-3)
        .map((d) => `Enhance ${d.name} capabilities`);
      if (latestResume.certifications?.length === 0)
        improvement_areas.push("Lack of formal certifications");

      // 5. Predicted Paths
      let predicted_paths = [];
      if (technical_score > 80 && rawText.includes("data"))
        predicted_paths = [
          "Senior Data Scientist",
          "Machine Learning Engineer",
          "AI Researcher",
        ];
      else if (technical_score > 80 && rawText.includes("react"))
        predicted_paths = [
          "Frontend Architect",
          "Full Stack Lead",
          "UI/UX Engineer",
        ];
      else if (leadership_potential > 80)
        predicted_paths = [
          "Engineering Manager",
          "Product Manager",
          "Tech Lead",
      ...
      if (leadership_potential < 60)
        growth_suggestions.push(
          "Take ownership of a small project or mentor junior peers to build leadership skills.",
        );
      if (communication_score < 70)
        growth_suggestions.push(
          "Consider writing technical blogs or presenting at meetups to boost visibility.",
        );
      if (technical_score < 70)
        growth_suggestions.push(
          "Deepen your expertise by contributing to open source or building complex side projects.",
        );
      growth_suggestions.push(
        `Leverage your strong ${top_strengths[0].toLowerCase()} to pivot towards ${predicted_paths[0]} roles.`,
      );
      ...
      if (insertError) {
        console.error(
          "[Database Error] Failed to save career twin report:",
          insertError,
        );
        return res
          .status(500)
      ...
      return res
        .status(500)
        .json({ message: "Failed to generate career twin." });
    }
  },
);
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (error && error.code !== "PGRST116") {
        ...
        return res
          .status(500)
          .json({ message: "Failed to fetch career twin report." });
      }

      if (!report) {
        return res
          .status(404)
          .json({ message: "No career twin report found." });
      }
      ...
      return res
        .status(500)
        .json({ message: "Server error fetching career twin report." });
    }
  },
);
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (
        ...
        questions.push({
          type: "Technical",
          text: `How would you optimize a machine learning model deployed at ${company} to handle high traffic inference without latency spikes?`,
        });
      } else if (role.toLowerCase().includes("front")) {
        questions.push({
          type: "Technical",
          text: `Explain how you would architect a highly performant and accessible React application for ${company}'s core product.`,
        });
      } else {
        questions.push({
          type: "Technical",
          text: `Describe your approach to designing a scalable distributed system for a high-availability service at ${company}.`,
        });
      }
      ...
      questions.push({
        type: "Coding",
        text: `Write an algorithm to detect cycles in a directed graph. Explain its time and space complexity, and why it matters for ${company}.`,
      });
      ...
      if (experience.length > 1) {
        questions.push({
          type: "Behavioral",
          text: `Tell me about a time you had a disagreement with a senior stakeholder. How did you resolve it and what was the outcome?`,
        });
      } else {
        questions.push({
          type: "Behavioral",
          text: `Describe a time when you had to learn a complex new technology very quickly under pressure.`,
        });
      }
      ...
      questions.push({
        type: "HR",
        text: `Why do you want to join ${company} specifically as a ${role}, and where do you see your career going in the next 3 years?`,
      });

      // Project-based Question (Based on specific skills)
      const primarySkill =
        skills.length > 0 ? skills[0] : "your primary technical stack";
      questions.push({
        type: "Project-based",
        text: `Walk me through the most complex project you built using ${primarySkill}. What were the biggest technical hurdles?`,
      });
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    if (!sessionId || !answers || !Array.isArray(answers)) {
      return res
        .status(400)
        .json({ message: "Session ID and an array of answers are required." });
    }
    ...
    try {
      ...
      for (const ans of answers) {
        ...
        if (wordCount < 10) {
          ...
          cScore = 30;
          suggestion =
            "Your answer is too short. Please provide the STAR (Situation, Task, Action, Result) method to give more context.";
        } else if (wordCount < 30) {
          qScore = 50;
          cScore = 60;
          suggestion =
            "You have the right idea, but try to expand on your specific actions and the measurable outcomes of your work.";
        } else {
          qScore = Math.min(
            100,
            60 + (wordCount > 60 ? 30 : 15) + Math.random() * 10,
          ); // Simulated AI heuristics
          cScore = Math.min(
            100,
            70 + (wordCount > 60 ? 20 : 10) + Math.random() * 10,
          );
          ...
          if (
            ...
            qScore -= 20;
            suggestion =
              "Avoid saying 'I don't know'. Instead, walk through how you would find the answer or solve the problem.";
          } else {
            suggestion =
              "Good use of detail. To improve further, ensure you quantify your achievements with metrics where possible.";
          }
        }
      ...
      return res
        .status(500)
        .json({ message: "Failed to evaluate interview answers." });
    }
  },
);
