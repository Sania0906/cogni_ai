    ...
    async function loadSkills() {
      try {
        ...
        if (data && data.length > 0) {
          ...
          const assessSum = assessmentsData.reduce(
          ...
          const avg = Math.round(
            assessmentsData.reduce((sum, item) => sum + item.score, 0) /
      ...
      <div className="flex items-start justify-between mb-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Track and improve your abilities
          </p>
        </div>
        <Link
          to="/skills/add"
              ...
              <div className="flex justify-between items-center mb-4">
                <div>
                  <h3 className="font-bold text-base text-foreground">
                    Gap Analysis: {gapData.targetRole}
                  </h3>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Required vs current levels to land this role
                  </p>
                </div>
                  ...
                  return (
                    <div key={s.name} className="space-y-1.5">
                      <div className="flex justify-between text-xs">
                        ...
                        <span
                          className={`font-bold ${isCritical ? "text-destructive" : isNeedsWork ? "text-warning" : "text-success"}`}
                        >
                      ...
                      <div className="h-2 rounded-full bg-muted overflow-hidden flex gap-0.5 relative">
                        ...
                        {s.gap < 0 && (
                          <div
                            ...
                            style={{
                              width: `${Math.min(100 - s.current, Math.abs(s.gap))}%`,
                            }}
                          />
                        )}
                      </div>
                  ...
                  >
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-4 min-w-0">
                        <div
                          className={`h-11 w-11 rounded-xl ${s.color} flex items-center justify-center text-white shrink-0`}
                        >
            ...
            <div className="rounded-3xl p-5 bg-card shadow-card border border-border/15">
              ...
              <div className="space-y-3">
                {assessments.map((a: any) => (
                  <div
                    key={a._id}
                    className="flex justify-between items-center p-3.5 rounded-2xl bg-muted/40 border border-border/15"
                  >
                    <div>
          ...
          {gapData && gapData.skills.some((s: any) => s.status !== "Met") && (
            <div className="rounded-3xl p-5 bg-card shadow-card border border-border/15">
              <h3 className="font-bold text-base text-foreground mb-3 flex items-center gap-1.5">
                <Sparkles className="h-4.5 w-4.5 text-primary" /> Learning
                Recommendations
              </h3>
              <div className="space-y-3">
                {gapData.skills
                  .filter((s: any) => s.status !== "Met")
                  .slice(0, 3)
                  .map((s: any) => {
                    let recCourse = "Advanced " + s.name;
                    if (s.name.includes("SQL"))
                      recCourse = "SQL & Database Administration";
                    if (s.name.includes("Python"))
                      recCourse = "Python for Data Science Foundations";
                    if (s.name.includes("MLOps"))
                      recCourse = "MLOps: Deploying ML Models to Production";
                    if (s.name.includes("React"))
                      recCourse = "React Development & Architectures";
                    if (s.name.includes("Cloud"))
                      recCourse = "Cloud Architectures & Microservices";
                    if (s.name.includes("Security"))
                      recCourse = "Network Security & Penetration Testing";

                    return (
                      <div
                        key={s.name}
                        className="p-4 rounded-2xl bg-muted/30 border border-border/10"
                      >
                        <div className="flex items-center gap-1.5">
                          <AlertCircle className="h-3.5 w-3.5 text-warning shrink-0" />
                          <span className="text-[10px] uppercase tracking-wider font-extrabold text-warning">
                            Focus Area: {s.name} Gap
                          </span>
                        </div>
                        <p className="text-sm font-bold mt-1 text-card-foreground">
                          {recCourse}
                        </p>
                        <Link
                          to="/courses"
                          className="text-xs text-primary font-bold inline-block mt-2 hover:underline"
                        >
