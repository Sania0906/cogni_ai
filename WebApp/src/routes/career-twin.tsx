...
function CareerTwin() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.error(
        err.message ||
          "Failed to generate Career Twin. Please ensure you have uploaded a resume.",
      );
    } finally {
      setAnalyzing(false);
    }
      ...
      <header className="mb-8 flex justify-between items-end">
        <div>
          ...
          <p className="text-muted-foreground mt-2 max-w-xl">
            A dynamic digital representation of your professional identity.
            Extracted entirely from your latest resume structure, linguistics,
            and experience map.
          </p>
        </div>
        {!analyzing && (
      ...
      {analyzing ? (
        <div className="h-[500px] flex flex-col items-center justify-center p-8 rounded-3xl bg-card border border-border/10 shadow-card">
          ...
          <p className="mt-2 text-sm text-muted-foreground text-center">
            Parsing technical stacks, inferring soft skills from semantics, and
            mapping maturity.
          </p>
        </div>
      ) : report ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="col-span-1 lg:col-span-3">
            <div className="p-8 rounded-3xl bg-gradient-primary text-white shadow-glow relative overflow-hidden flex flex-col md:flex-row items-center md:items-start gap-8">
              ...
              <div className="flex-1 relative z-10 text-center md:text-left">
                <h3 className="text-sm font-extrabold tracking-widest uppercase text-white/80 mb-1">
                  Career Personality
                </h3>
                ...
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 text-sm font-bold backdrop-blur-sm mb-4">
                  <Briefcase className="h-4 w-4" /> Maturity Level:{" "}
                  {report.maturity_level}
                </div>
                <p className="text-white/90 max-w-2xl text-sm leading-relaxed">
                  Your twin reflects a profile weighted strongly toward your
                  parsed experience. Your predicted pathways highlight alignment
                  with {report.predicted_paths?.[0]} and similar roles.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-1">
            <div className="p-6 rounded-3xl bg-card border border-border/10 shadow-card h-full">
              ...
              <ProgressBar
                label="Soft Skills"
                score={report.soft_skills_score}
                icon={MessageSquare}
                colorClass="text-pink"
              />
              <ProgressBar
                label="Leadership Potential"
                score={report.leadership_potential}
                icon={Shield}
              ...
              <ul className="space-y-3">
                {report.top_strengths?.map((s: string, i: number) => (
                  <li key={i} className="flex gap-3 text-sm items-start">
                    <span className="h-5 w-5 rounded-full bg-success/10 text-success flex items-center justify-center shrink-0 text-xs font-bold mt-0.5">
                      {i + 1}
                    </span>
            ...
            <div className="p-6 rounded-3xl bg-card border border-border/10 shadow-card">
              ...
              <ul className="space-y-3">
                {report.improvement_areas?.map((s: string, i: number) => (
                  <li key={i} className="flex gap-3 text-sm items-start">
                    <span className="h-5 w-5 rounded-full bg-destructive/10 text-destructive flex items-center justify-center shrink-0 text-xs font-bold mt-0.5">
                      {i + 1}
                    </span>
                    <span className="font-medium text-foreground">{s}</span>
                  </li>
            ...
            <div className="p-6 rounded-3xl bg-card border border-border/10 shadow-card bg-primary/5 border-primary/10">
              ...
              <div className="flex flex-wrap gap-2">
                {report.predicted_paths?.map((path: string, i: number) => (
                  <span
                    key={i}
                    className="px-3 py-1.5 rounded-lg bg-background border border-border/50 text-sm font-bold shadow-sm"
                  >
                    {path}
                  </span>
                ))}
            ...
            <div className="p-6 rounded-3xl bg-card border border-border/10 shadow-card">
              ...
              <div className="space-y-4">
                {report.growth_suggestions?.map((s: string, i: number) => (
                  <div
                    key={i}
                    className="p-3 rounded-xl bg-muted/50 border border-border/30 text-sm leading-relaxed"
                  >
                    {s}
                  </div>
                ))}
        ...
        <div className="h-[400px] flex flex-col items-center justify-center p-8 rounded-3xl bg-card border border-dashed border-border/40 text-center">
          ...
          <h3 className="text-lg font-bold text-foreground">
            No Twin Generated Yet
          </h3>
          <p className="text-sm text-muted-foreground max-w-sm mt-2 mb-6">
            Generate your AI Career Twin to unlock personalized insights and
            roadmap predictions based on your latest resume upload.
          </p>
          <button
            onClick={handleGenerate}
