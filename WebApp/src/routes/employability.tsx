      ...
      <AppShell>
        ...
        <div className="rounded-3xl p-6 bg-card border border-destructive/20 text-center space-y-4 max-w-sm mx-auto mt-10">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            ...
            Skill test) to unlock your Employability Index score!
          </p>
        </div>
      </AppShell>
    );
    ...
    <AppShell>
      ...
      <div className="p-6 rounded-3xl bg-card shadow-card flex flex-col items-center mb-6">
        ...
        <p className="font-bold text-center text-base">
          Market Readiness Status: Highly Competitive
        </p>
        <p className="text-xs text-muted-foreground mt-1 text-center">
          Your profile matches standard candidate benchmarks.
        </p>
      </div>

      {/* Component breakdown */}
      <h3 className="text-base font-bold mb-3 flex items-center gap-1.5">
        <ShieldCheck className="h-5 w-5 text-primary" /> Score Breakdown
      </h3>
        ...
        {data.components.map((c) => (
          ...
          >
            ...
            <div className="h-2 w-full bg-muted rounded-full overflow-hidden mb-2">
              <div
                className={`h-full ${getColor(c.score)} rounded-full`}
                style={{ width: `${c.score}%` }}
              />
            </div>
